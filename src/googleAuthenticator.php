 ...
 *
 * project Url: https://github.com/PHPGangsta/GoogleAuthenticator
 *
 * use: $secret = GoogleAuthenticator::createSecret();
 * use: $qrCodeUrl = GoogleAuthenticator::getQRCodeGoogleUrl("Blog", $secret);
 * use: $oneCode = GoogleAuthenticator::getCode($secret);
 * use: $checkResult = GoogleAuthenticator::verifyCode($secret, $oneCode, 2);
 *
 */
...
class GoogleAuthenticator {
	...
	public static function getQRCodeGoogleUrl($name, $secret, $title = null, $params = []) {
		$width = !empty($params["width"]) && (int) $params["width"] > 0 ? (int) $params["width"] : 200;
		$height = !empty($params["height"]) && (int) $params["height"] > 0 ? (int) $params["height"] : 200;
		$level = !empty($params["level"]) && array_search($params["level"], ["30", "25", "15", "7"]) !== false ? $params["level"] : "15";

		$urlencoded = urlencode("otpauth://totp/" . $name . "?secret=" . $secret);
		if (isset($title)) {
			$urlencoded .= urlencode("&issuer=" . urlencode($title));
		}
		return "https://tool.lu/qrcode/basic.html?text=" . $urlencoded . "&tolerance=" . $level . "&size=" . $width;
	}
